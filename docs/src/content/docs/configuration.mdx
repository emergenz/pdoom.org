...
<Aside type="caution">
  crowd-cast reads its config at startup only, so quit crowd-cast before editing
  the config file and restart it afterwards. (Changing the recorded apps from the
  tray **Settings** dialog instead applies right away, restarting recording with
  the new selection.)
</Aside>
